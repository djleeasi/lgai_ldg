...
class customloss():
    ...
    def __call__(self, gt, pred):
        ...
        return 1.2*torch.sum(nrmse[:8])+1*torch.sum(nrmse[8:14])
    #deprecated
    def origin(self,tensor):
        return tensor*(self.max-self.min)+self.min 
    def MSEfunc(self, gt, pred):
        subtract = torch.sub(gt,pred)
        meaned = torch.mean(torch.square(subtract),0)

        #div = subtract/gt.shape[0]
        #print(div, div.shape)
        return meaned
